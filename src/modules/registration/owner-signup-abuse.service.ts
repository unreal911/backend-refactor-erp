...
export class SignupAbuseReviewService {
    async list(input: {
        page: number;
        limit: number;
        reviewStatus?: SignupAbuseReviewStatus;
    }) {
        const where = input.reviewStatus
            ? { reviewStatus: input.reviewStatus }
            : {};
        const [events, total] = await platformPrisma.$transaction([
            platformPrisma.signupAbuseEvent.findMany({
                where,
                orderBy: [{ lastOccurredAt: "desc" }, { id: "desc" }],
                skip: (input.page - 1) * input.limit,
                take: input.limit,
            }),
            platformPrisma.signupAbuseEvent.count({ where }),
        ]);
        return {
            data: events.map((event) => ({
                id: event.id,
                reason: event.reason,
                captchaVerified: event.captchaVerified,
                ...
                reviewedByPlatformAdminId: event.reviewedByPlatformAdminId,
                overrideUntil: event.overrideUntil,
                identityReferences: {
                    email: event.emailFingerprint.slice(0, 12),
                    ip: event.ipFingerprint.slice(0, 12),
                    device: event.deviceFingerprint.slice(0, 12),
                },
            })),
            total,
            page: input.page,
    ...
    async review(input: {
        ...
        outcome: Extract<SignupAbuseReviewStatus, "FALSE_POSITIVE" | "CONFIRMED_ABUSE">;
        note: string | null;
        ...
        const updated = await platformPrisma.$transaction(async (tx) => {
            ...
            const reviewed = await tx.signupAbuseEvent.update({
                ...
                reviewedAt: now,
                reviewedByPlatformAdminId: input.platformAdminId,
